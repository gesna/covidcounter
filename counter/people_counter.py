...
while True:
	...
	# frame = open(image_path, "r+").read()
	frame2=open(image_path, "rb")
	...
	if totalFrames % args["skip_frames"] == 0 and wait == 0:
		...
		response = requests.post(
			analyze_url, headers=headers, params=params, data=cv2.imencode('.jpg', frame)[1].tostring())
		...
		for i in range(len(analyzed['objects'])):
			...
			rectangle = analyzed['objects'][i]['rectangle']
			rect = dlib.rectangle(rectangle['x'], rectangle['y'], rectangle['x']+rectangle['w'], rectangle['y']+rectangle['h'])
